...
def featurize(record: dict[str, Any]) -> str:
    ...
    lower = text.lower()
    meta = [
        "META_QMARK=" + ("yes" if "?" in text else "no"),
        "META_STARTS_AUX="
        + (
            "yes"
            if re.match(r"^\s*(is|does|do|did|should|can|could|would|are|was|were|will)\b", text, re.I)
            else "no"
        ),
        "META_STARTS_OPEN="
        + (
            "yes"
            if re.match(r"^\s*(explain|describe|provide|list|outline|state|confirm|summarize|why|how)\b", text, re.I)
            else "no"
        ),
        "META_STARTS_DO_NOT="
        + ("yes" if re.match(r"^\s*(do not|don't)\b", text, re.I) else "no"),
        "META_STARTS_GENERIC_SUBJ="
        + (
            "yes"
            if re.match(
                r"^\s*(the|this|that|it|many|people|some|most|drivers|winter|grilling|sleeping)\b",
                text,
                re.I,
            )
            else "no"
        ),
        "META_HAS_IF=" + ("yes" if " if " in f" {lower} " else "no"),
        "META_HAS_ALWAYS=" + ("yes" if " always " in f" {lower} " else "no"),
        "META_HAS_SHOULD_YOU_NOT="
        + (
            "yes"
            if re.search(r"\bshould you not\b|\bshouldn't you\b", text, re.I)
            else "no"
        ),
        "META_HAS_DO_NOT_HAVE=" + ("yes" if "do not have" in lower else "no"),
        "META_HAS_NOT_AVAILABLE=" + ("yes" if "not available" in lower else "no"),
        "META_HAS_NOT_A_AN="
        + (
            "yes"
            if re.search(r"\bis not a\b|\bis not an\b", text, re.I)
            else "no"
        ),
    ]
    return "\n".join(
...
def predict_split(
    ...
    for record in records:
        ...
        if llm_token == "[SELECT]" and rule_token == "[PRESERVE]":
            ...
            details[rid] = {"route": "rule_preserve"}
            continue

        if llm_token == "[SELECT]" and rule_token == "[SUPPRESS]":
            suppress_prob = float(suppress_model.predict_proba([featurize(record)])[0][1])
            if suppress_prob >= threshold:
                predictions[rid] = "[SUPPRESS]"
                details[rid] = {"route": "suppress_rescue", "suppress_prob": suppress_prob}
            else:
                predictions[rid] = ""
                details[rid] = {"route": "keep_select_empty", "suppress_prob": suppress_prob}
            continue

        predictions[rid] = "" if llm_token == "[SELECT]" else llm_token
        details[rid] = {"route": "base_llm_or_empty"}
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/router_suppress_rescue")
    ...
    train_branch = [
        ...
        if llm_train[r["id"]] == "[SELECT]" and predict_behavior(r) == "[SUPPRESS]"
    ]
    suppress_model = make_model(C=1.5)
    suppress_model.fit(
        [featurize(r) for r in train_branch],
        [int(r["expected_neg_behavior"] == "suppress_target") for r in train_branch],
    )

    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    report: dict[str, Any] = {
        "train_branch_size": len(train_branch),
        "train_branch_gold": dict(Counter(r["expected_neg_behavior"] for r in train_branch)),
        "thresholds": {},
    }

    for threshold in args.thresholds:
        key = f"{threshold:.2f}"
        dev_predictions, dev_details = predict_split(dev_records, llm_dev, suppress_model, threshold)
        test_predictions, test_details = predict_split(test_records, llm_test, suppress_model, threshold)
